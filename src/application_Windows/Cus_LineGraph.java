...
public class Cus_LineGraph extends JPanel{
	...
	private ButtonGroup group; //JRadioButton are put into this group, so that one can be selected each time
	...
	private JPanel controls; //a control panel that will hold all the controls components

	/**
	 * Constructor method
	 * @param window a reference to the main frame
	 * @param fileDownloader a reference to the fileDownloader class
	 */
	 ...
	 * @param array change the data based on the array
	 * @param x the x position of the created component
	 * @param y the y position of the created component
	 * @param width the width of the created component
	 * @param height the height of the created component
	 * @return return the created styled component
	 */
	 ...
	 * @param init the default selection mode
	 * @param x the x position of the created component
	 * @param y the y position of the created component
	 * @param width the width of the created component
	 * @param height the height of the created component
	...
	private JLabel makeLabel(String text, int x, int y, int width, int height){
		JLabel label = new JLabel(text); //initialize the label based on its text
	 ...
	 * @param space the spacing between each ticks
	 * @param x the x position of the created component
	 * @param y the y position of the created component
	 * @param width the width of the created component
	 * @param height the height of the created component
	...
	private JSlider makeSlider(int min, int max, int value, int space, int x, int y, int width, int height){
		...
		//set the UI later because the slider UI have to use static variables for the slider, if the slider is not yet created, it will show an error 
	 ...
	 * 
	 * @param sliderMonth the slider that controls the month range of the data
	 * @param sliderYear the slider that controls the year range of the data
	 * @param citySelection the comboBox that will be used for city selection
	 * @param dataSelection  the comboBox that will be used for data type selection
	 */
	 ...
	 * @param city  the city that hold the current displaying city
	 * @param sliderYear the slider that can shows the year of the data
 	 */
	...
	/**
	 * change the data based on the data selected and also the current city selected
	 * @param index index of the selected item
	 * @param city the current city selected
	 */
	 ...
	 * 
	 * @param monitor most likely the monthly slider 
	 * @param masterSlider most likely the yearly slider
	 */
	 ...
	 * @param year the year of the data range
	 */
	private void getData(String year){
		...
		for(String key: data.keySet()){ //for all data in the entire data set
			for(int yearCount = 1; yearCount<=12; yearCount++){ //for loop 12 times to illustrate the 12 months within a year
				String speDateMonth = year+"-"+String.format("%02d", yearCount); //temporarily store the String of year and month
	 ...
	 * 
	 * @param year the desired year range
	 * @param month the desired month range
	 */
	private void getData(String year, String month){
		draw.clear(); //clear the data before
		//for every key inside of the entire data's key set
	...
	/**
	 * get to initialize the indexes of x and y axis and calculate the pointed 
	 */
	private void init(){
	...
	public void paintComponent(Graphics g){
		...
		rotate.setToRotation(Math.toRadians(-90), startX-70, startY+title.length()*7); //set the rotation coordinates, based on the coordinate of the feature
		normal = g2d.getTransform(); //get the current transform position so that it can be transform back later
		g2d.setTransform(rotate); //set the transform to the rotated position

		g2d.drawString(title, startX-70, startY+title.length()*7); //draw the title again to indicate what the y-axis will be
		g2d.setTransform(normal);//set the transform back to normal
		...
		for(int i = 0; i<=keySetDate.size()-1; i++){
			...
			if(i%(int)xValueDiff==0){ //if it is a major tick
				...
				g2d.setTransform(rotate); //set the transform to rotated to the target position
		...
		// have a temporarily variable that will store the previous dot to connect the lines
		...
		for(String key: draw.keySet()){
			...
			float yCoord = startY+height-yIndexDistance-(yValue-minY)*yScale; //calculate the y position based on the y value
			g2d.setColor(Color.white); //set the color to white
			if( draw.keySet().size()<=31){ //if the draw data is out monthly
				g2d.drawRect((int)xCoord-4, (int)yCoord-4, 8, 8); //bigger coordinates
			...
			beforeYCoord = (int)yCoord; //save the currentY as before
		}
